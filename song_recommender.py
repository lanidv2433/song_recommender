...
def run():
    ...
    while again:
        ...
        results = sp.search(q='artist:' + artist + ', track:' + title, limit = 15, type="track")
        ...
        if results['tracks']['total'] == 0:
            ...
            recommendations = sp.recommendations(seed_tracks=[song['uri']])
            recDic = {}
            for num, track in enumerate(recommendations['tracks'], 1):
                print(f"{num}. {track['name']} - {track['artists'][0]['name']}")
              
                recDic[num] = [track['preview_url'], f"{track['name']} by {track['artists'][0]['name']}"]
